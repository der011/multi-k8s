...
kubectl set image deployments/client-deployment server=der011/multi-client:$SHA
kubectl set image deployments/server-deployment server=der011/multi-server:$SHA
kubectl set image deployments/worker-deployment server=der011/multi-worker:$SHA
